...
export const SearchResults: React.FC<SearchResultsProps> = memo(
  ({ query, results, loading, onProductPress, onBackToSearch }) => {
    ...
    SearchResults.displayName = "Search Result";
    ...
    const renderSortOption = useCallback(
      ({ item }: { item: (typeof sortOptions)[0] }) => (
        <Pressable
          style={[
            styles.sortOption,
        ...
        <View style={styles.header}>
          ...
          <View style={styles.headerContent}>
            ...
            <Text style={styles.resultCount}>
              {finalResults.length}{" "}
              {finalResults.length === 1 ? "result" : "results"}
            </Text>
          </View>
          <Pressable
